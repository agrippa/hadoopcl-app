 ...
 */
public class ASFTxtToSeq {
    public static void main(String[] args) throws Exception {
        if (args.length != 3) {
            System.out.println("usage: java ASFTxtToSeq input-dirname output-dirname n-input-files");
            return;
        }
        ...
        int nThreads = 12;
        int chunkSize = (nInputFiles + nThreads - 1) / nThreads;
    ...
    public static class WikiRunner implements Runnable {
        ...
        public void run() {
            for (int i = start; i < end; i++) {
                String contents;
                try {
                    contents = FileUtils.readFileToString(new File(inputDirName+"/"+i), charset);
                    writer.write(Integer.toString(i), contents);
                } catch(FileNotFoundException fnf) {
                    // do nothing, skip any input files we don't find, not sure why they aren't there but it doesn't really matter
