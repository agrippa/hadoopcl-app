...
#ssh -o ConnectTimeout=2 ${n} "grep -n -R 'DIAGNOSTICS' /tmp/${PBS_JOBID}/hadoop-jmg3/mapred/local/userlogs/* > ${HADOOP_LOG_DIR}/diagnostics.${BENCHMARK}.${FORMAT}.${MAP_OUTPUTFORMAT}.opencl.${PBS_JOBID}.${mapper}.${CPU_GROUP}.${GPU_GROUP}.${CPU_THREAD}.${GPU_THREAD}.${buffer}.${hdfs_chunk_size}.${n}"
    done
    echo Done, Killing
    ./KILL.sh
